# ACTA TEE Assistant — native iOS SwiftUI app for CT anesthesiology fellows
...
2. `xcodegen generate` in this directory → produces `ACTATEEAssistant.xcodeproj`.
3. Open `ACTATEEAssistant.xcodeproj`, select the ACTATEEAssistant scheme, run on an iPhone
   simulator/device (iOS 17+).
4. Unit tests: `xcodebuild test -scheme ACTATEEAssistant -destination 'platform=iOS Simulator,name=iPhone 15'`
...
- F3 Spaced-repetition quiz — done (SM-2 scheduler + question generator in
  ACTATEEAssistantCore; QuizCard model; Review · N cards due entry on the Library
...
- F1 Siri Shortcuts / hands-free capture — done (LogTEECaseIntent,
  ...
  WidgetKit extension showing total/totalMinimum with actatee://quicklog
