// ACTATEEProgressWidget.swift — WidgetKit complication: total/totalMinimum
...
struct ACTATEEProgressEntry: TimelineEntry {
    let date: Date
    let total: Int
...
struct ACTATEEProgressProvider: TimelineProvider {
    func placeholder(in context: Context) -> ACTATEEProgressEntry {
        ACTATEEProgressEntry(
            date: .now,
            total: 247,
    ...
    func getSnapshot(in context: Context, completion: @escaping (ACTATEEProgressEntry) -> Void) {
        completion(loadEntry())
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<ACTATEEProgressEntry>) -> Void) {
    ...
    /// call).
    private func loadEntry() -> ACTATEEProgressEntry {
        ...
        return ACTATEEProgressEntry(date: .now, total: total, totalMinimum: track.totalMinimum, trackName: track.name)
    }
...
struct ACTATEEProgressWidgetView: View {
    @Environment(\.widgetFamily) private var family
    let entry: ACTATEEProgressEntry
...
struct ACTATEEProgressWidget: Widget {
    static let kind = "ACTATEEProgress"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: Self.kind, provider: ACTATEEProgressProvider()) { entry in
            ACTATEEProgressWidgetView(entry: entry)
                .widgetURL(URL(string: "actatee://quicklog")) // tap-through → Quick Log
        }
        .configurationDisplayName("ACTA TEE Progress")
...
@main
struct ACTATEEWidgetBundle: WidgetBundle {
    var body: some Widget {
        ACTATEEProgressWidget()
    }
}
